class GameState {
    ...
    constructor(engine) {
        this.engine = engine;
        Object.assign(this, engine.gameProperties);
    }

    // updateScene() {
    //     for (let key in this.engine.gameModel.properties) { // Update game properties
    //         this["_" + key] = this.engine.gameModel.properties[key];
    //     }
    // }

    // Render properties
    get displayWidth() { return this._displayWidth };
    set displayWidth(value) { this._displayWidth = value; this.engine.render.renderer.resize(value, this.displayHeight) }

    get displayHeight() { return this._displayHeight };
    set displayHeight(value) { this._displayHeight = value; this.engine.render.renderer.resize(this.displayWidth, value) }
    ...
    set backgroundColor(value) {
        this._backgroundColor = value; this.engine.render.renderer.backgroundColor = PIXI.utils.string2hex(value);
        this.engine.render.update(this.engine);
    }
    ...
    set soundtrack(value) {
        ...
        if (value) {
            if (this.engine.audio.music) this.engine.audio.music.source.stop(this.engine.audio.music.id);
            this.engine.audio.music = new Sound(value);
            this.engine.audio.music.source.play(this.engine.audio.music.id);
        }
    }
    ...
    set loop(value) {
        this._loop = value;
        if (this.engine.audio.music) {
            this.engine.audio.music.source.loop(value);
            this.engine.audio.music.source.play();
        }
    }
    ...
    // falta definir las acceleraciones
}